...
class imagesFolder:    
    ...
    def __init__(self , window: QWidget, main_window: QMainWindow , scroll: QScrollArea , config: ConfigParser) -> None:
    ...
    def start(self , label_to_change: QLabel) -> True:
        ...
        # Set Stylesheet
        self.pinned.setStyleSheet("""color: {}; font-family: {}; font-size: {};""".format(
        ...
        # Change Text
        self.pinned.setText(self.config.get("folderPage" , "pinnedText-icon")[1:-1] + "Pinned")
        ...
        # Change StyleSheet
        self.folderHeaderText.setStyleSheet("""color: {}; font-family: {}; font-size: {};""".format(
        ...
        # Change Text
        self.folderHeaderText.setText(self.config.get("folderPage" , "restFolders-icon")[1:-1] + "Folders")
        ...
        height , width = int(self.config.get("folderPage" , "folders-height")) , int(self.config.get("folderPage" , "folders-width"))
        ...
        self.keybindings = json.loads(self.config.get("folderPage" , "folderPage-keybindings"))
        ...
        for i in self.dirs:
            ...
            if(os.path.isdir(curr) and i[0] != '.' and curr not in self.prevented_dirs):                
                ...
                if(curr in self.currPinned):
                    res = self.update(curr , px , py , curr in self.currPinned , colors[color_pinned])
                else:  
                    res = self.update(curr , x , y , curr in self.currPinned , colors[color_rest])
    ...
    def pushUpward(self) -> True:
        ...
        self.info = QLabel(text="  No Folders Pinned. Double Click On The Folder To Pin" , parent=self.window)
        ...
        try:
            self.animations.addAnimation(Animation.movingAnimation(Animation , self.folderHeaderText , QPoint(self.folderHeaderText.pos().x(), self.folderHeaderText.pos().y() + 1) , 400))
        except:
           pass
                
        # Iterate through all the folders to move them up
        for i in self.allFolders:
            self.animations.addAnimation(Animation.movingAnimation(Animation , i , QPoint(i.pos().x() , i.pos().y() - 200) , 400))
    ...
    def update(self , dir: str , x: int , y: int , is_pinned: bool , color: str) -> bool: 
        ...
        # Set some styles
        folderName.setStyleSheet('color: {}; font-family: {}; font-size: {}px; padding-top: {}px'.format(
    ...
    def get_first(self , dir: str) -> str:
        ...
        for i in os.listdir(dir):
            ...
            if(i[-3:] in ['png' , 'jpeg' , 'jpg' , 'webp'] and not os.path.isdir("{}/{}".format(dir , i))):
                return "{}/{}".format(dir , i)
    ...
    def removeFromPin(self , dir: str) -> None:
        ...
        self.remove_pinning_animation.addAnimation(Animation.movingAnimation(Animation , dir , QPoint(40 , self.folderStartValue) , 200))
    ...
    def addToPinned(self , dir: QDoublePushButton):
        ...
        self.pinning_animations.addAnimation(Animation.movingAnimation(Animation , dir , QPoint(40 , 220) , 200))
    ...
    def responser(self , event=None):
        ...
        card_height , card_width , padding = int(self.config.get("folderPage" , "folders-height")) , int(self.config.get("folderPage" , "folders-width")) , int(self.config.get("folderPage" , "folders-padding"))
        
        # Hide the messages if the folders is not empty
        if(self.allFolders != []): self.hide_msg()
        ...
        for i in self.folders_pinned:          
            self.an.addAnimation(Animation.movingAnimation(Animation , i , QPoint(x , y) , 200))
        ...
        try:
            self.an.addAnimation(Animation.movingAnimation(Animation , self.folderHeaderText , QPoint(40 , self.folderStartValue + 300) , 100))
    ...
    def pushDown(self):
        ...
        self.anim.addAnimation(Animation.movingAnimation(Animation , self.folderHeaderText , QPoint(40 , self.folderHeaderText.pos().y() + 400) , 200))
    ...
    def showMessage(self):        
        # Create a msg label
        self.msg = QLabel(parent=self.window , text="Nothing Here  ! Probably You Have Pinned All The Folders Or Didn't Select Any Folder")
    ...
    def transfer_control(self , directory): 
        ...
        self.effects.addAnimation(Animation.fadingAnimation(Animation , self.images , 400))

        self.effects.addAnimation(Animation.fadingAnimation(Animation , self.label_to_change , 400))
        
        self.effects.addAnimation(Animation.fadingAnimation(Animation , self.pinned , 400))
    ...
    def run_second(self , dir):  
        ...
        singleFolderView(self.window , dir , self.config , self.label_to_change , self.images , self.folderHeaderText , self.pinned , *self.args).start()
